...
public class JTAppleCalendarLayout: UICollectionViewLayout, JTAppleCalendarLayoutProtocol {
    ...
    public override func prepareLayout() {
        ...
        if delegate.headerViewXibs.count < 1 { headerCache.removeAll() } // Get rid of header data if dev didnt register headers. The were used for calculation but are not needed to be displayed
    ...
    override public func layoutAttributesForElementsInRect(rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for sectionIndex in startSectionIndex..<cellCache.count {
            if let validSection = cellCache[sectionIndex] where validSection.count > 0 {
                ...
                if delegate.headerViewXibs.count > 0 {
    ...
    func applyLayoutAttributes(attributes : UICollectionViewLayoutAttributes) {
        ...
        if delegate.headerViewXibs.count > 0 { // If we have headers the cell must start under the header
    ...
    func sizeForitemAtIndexPath(indexPath: NSIndexPath) -> CGSize {
        ...
        var headerSize =  CGSizeZero
        if delegate.headerViewXibs.count > 0 { headerSize = cachedHeaderSizeForSection(indexPath.section) }
